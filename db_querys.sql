...
create view v_clients_accounts as
	select
		...
		balance as saldo
	from clients cl
		left join accounts on cl.id = accounts.client_id
...
create view v_accounts_types as
	select
		...
		actp.name as "tipo de cuenta"
	from accounts acc
		left join account_types actp on acc.type = actp.id ;
...
create view v_transactions_detail as
	select 
		t.id as transacción,
		amount as monto,
		ttp.name as "tipo de transacción"
	from transactions t
		left join transactions_types ttp on t.transaction_type = ttp.id;
...
create view v_transations_types_detail as
	select
		ttp.id as transacción,
		t.amount as monto,
		name as "tipo de transacción"
	from transactions_types ttp
